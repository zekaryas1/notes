...
- The goal of TypeScript is to be a static typechecker for JavaScript programs
    - in other words, a tool that runs before your code runs (static) and ensure that the types of the program are
      correct (typechecked).
- TS = JavaScript + Types
    - *Basically TypeScript provides types for JavaScript, and understanding these types is the key to mastering
      TypeScript.*
...
> Type safety is one of the main motivations for using TypeScript and you should try to avoid using any when not
> necessary.
...
- TypeScript comes with a ReadonlyArray type that is the same as Array with all mutating methods removed, so you can
...
- Here we're saying a SquareConfig can have any number of properties, and as long as they aren't color or width, their
  types don't matter.
