...
- This project is a small RAG system implementation, it process a file from a local path and lets you ask a question
...
- before we store our embeddings, we first need to configure, the store, and the embedding model we like to use
    - [sentence-transformers/all-MiniLM-L6-v2 · Hugging Face](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2)
      has 384 dimensional dense vector space.
...
- the llm library will check every row and calculate a cosine similarity score, we pass 3 as we only want the top 3
  results.
...
- we use a very small [orca-mini:3b](https://ollama.com/library/orca-mini:3b) (3B parameter and 2GB file size), the
  system prompt inform the model to consider the passed context only.
