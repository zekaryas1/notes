...
2. **Collaborative Filtering**
    ...
    - Uses user preferences (e.g., likes, ratings) to identify similar users and suggest items the target user hasn’t
      interacted with.
...
2. **Sparse Vector**
    ...
    - **Use Cases**: Recommendation systems (user-item matrices, where users interact with few items), NLP, network
      simulations.
...
- **Sparse**: Use when significantly more than half the elements are zero, especially for large datasets, to save memory
